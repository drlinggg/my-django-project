...
from expenses.serializers import ExpensesWriteSerializer, ExpensesReadSerializer

def get_expenses_with_filters(user, filters=None):
    """
...
def get_expense_by_id(user, expense_id):
    """
...
def update_expense(user, expense_id, data):
    ...
    serializer = ExpensesWriteSerializer(expense, data=data, partial=True)
    if not serializer.is_valid():
        raise ValidationError(serializer.errors)
